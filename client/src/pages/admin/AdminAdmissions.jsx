...
const AdminAdmissions = () => {
  ...
  useEffect(() => {
    ...
    api.get('/admissions').then(({ data }) => {
      ...
      const admission = list.find(a => a.user_id === location.state.userId);
      if (admission) {
        setViewingAdmission(admission);
        navigate(location.pathname, { replace: true, state: {} });
      }
    });
  }, [location.state, location.pathname, navigate, refreshKey]);
  ...
  const columns = [
    ...
    { source: 'courses.name', label: 'Course' },
    {
      ...
      label: 'Status',
      sortable: true,
      render: (v) => <span className={`badge badge-${v === 'approved' ? 'success' : v === 'rejected' ? 'danger' : 'warning'}`}>{v}</span>,
    },
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        rowActions={(a) => (
          <>
            <button onClick={() => setViewingAdmission(a)} className="btn-icon" title="View details" style={{ background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', padding: '0.25rem' }}>
              <Eye size={18} />
            </button>
            {a.status === 'pending' && (
              <>
        ...
        <div className="modal-overlay" style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60 }}>
          <div className="modal-content card" style={{ width: '100%', maxWidth: '400px', padding: '2rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
              ...
              <button onClick={() => handleStatus(approvingAdmission.id, 'approved', selectedSession)} className="btn btn-primary" disabled={processing === approvingAdmission.id}>
                        ...
                        <div key={doc.id} style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', padding: '0.75rem', border: '1px solid var(--gray-200)', borderRadius: '0.5rem' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <div style={{ display: 'flex', gap: '0.5rem' }}>
                              {(isImage || isPdf) && (
                                <button onClick={() => setPreviewDocId(isPreviewing ? null : doc.id)} className="btn-icon" style={{ padding: '0.5rem', background: 'var(--gray-100)', color: 'var(--gray-700)', borderRadius: '0.375rem', display: 'flex', alignItems: 'center', border: 'none', cursor: 'pointer' }} title="Preview">
                                  <Eye size={16} />
                                </button>
                              )}
                          ...
                          {isPreviewing && (
                            <div style={{ marginTop: '0.5rem', borderTop: '1px solid var(--gray-200)', paddingTop: '0.75rem' }}>
                              {isImage ? <img src={doc.previewUrl || doc.downloadUrl} alt={doc.original_name} style={{ width: '100%', maxHeight: '400px', objectFit: 'contain', borderRadius: '0.375rem' }} /> : isPdf ? <iframe src={doc.previewUrl || doc.downloadUrl} title={doc.original_name} style={{ width: '100%', height: '400px', border: 'none', borderRadius: '0.375rem' }} /> : null}
                            </div>
                          )}
                        </div>
                      );
